...
Почему натрий — это металл, а неон — газ? Ответ кроется в развернутом наружу «фонтане».
...
### 2. Проводимость
По этим открытым каналам эфир течет свободно, что и обеспечивает высокую электропроводность натрия.
...
- **Фтор (Хищник):** Его ядро (4α) — незавершенная Т-форма. Тритон там пытается закрыть дыру, создавая тягу эфира **внутрь**. Фтор хочет забрать чужое.
- **Натрий (Донор):** Его ядро (5α) уже завершено. Тритон для него — **лишний балласт**. Повернутая альфа-частица создает избыточное давление, которое натрий стремится **сбросить**.
...
Атомы держатся друг за друга только слабыми «хвостами», металлическая решетка очень гибкая.

**Реальность:** Натрий можно резать обычным ножом. ✓

### Прогноз №2: Низкий потенциал ионизации
«Лишний» электрон от повернутой альфы выталкивается всей мощью неонового каркаса.

**Реальность:** Вырвать электрон из натрия в 4 раза легче, чем из неона. ✓

### Прогноз №3: Низкая температура плавления
...
В следующей части мы добавим ещё один элемент.
- **Магний:** как симметрия возвращается через парность.
- Почему Магний двухвалентен и откуда берется его прочность?
...
[https://3d-particles-pi.vercel.app/](https://3d-particles-pi.vercel.app/)
